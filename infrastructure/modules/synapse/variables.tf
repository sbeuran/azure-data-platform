...
variable "synapse_public_network_enabled" {
  description = "Enable public network access for Synapse"
  type        = bool
  default     = false
}
...
variable "synapse_sql_pool_sku" {
  ...
  validation {
    condition     = can(regex("^DW[0-9]+c$", var.synapse_sql_pool_sku))
    error_message = "Synapse SQL pool SKU must be in format DW{number}c."
  }
}
